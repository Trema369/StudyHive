    ...
    <main className="mx-auto max-w-7xl min-h-screen overflow-y-auto p-6 py-30 space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-[20rem_1fr]">
        <div className="rounded-lg border p-2 space-y-2 max-h-[42rem] overflow-y-auto">
          {chats.map((chat) => {
            ...
            return (
              ...
              >
                <div className="font-medium">{chat.name ?? "Direct chat"}</div>
        ...
        <div className="rounded-lg border p-4 space-y-3">
          {!selectedChat ? (
            <p className="text-sm text-muted-foreground">Select a chat...</p>
          ) : (
            <>
              ...
              {tab === "chat" && (
                <>
                  ...
                  {replyToMessageId && messageById[replyToMessageId] && (
                    <div className="rounded-md border bg-muted/40 p-2 text-xs text-muted-foreground">
                      Replying to{" "}
                      {displayName(messageById[replyToMessageId].userId)}:{" "}
                      {(
                        splitThink(messageById[replyToMessageId].text)
                          .content ||
                        messageById[replyToMessageId].text ||
                        ""
                      ).slice(0, 120)}
                      <Button
                        size="sm"
                  ...
                  <div className="flex gap-2">
                    <Input
                      placeholder={
                        selectedChat.adminOnly && !isAdmin
                      ...
                      disabled={selectedChat.adminOnly && !isAdmin}
                    />
                    <Button
                      type="button"
                      size="icon"
                ...
                <div className="space-y-3">
                  ...
                  {isAdmin && (
                    <div className="flex gap-2">
                      ...
                      />
                      <Button onClick={() => void addUserToChat()}>
                ...
                <div className="space-y-3 rounded-md border p-3">
                  {isSelectedDirect ? (
                    <p className="text-sm text-muted-foreground">
                      Direct chats must not have a custom name.
                    </p>
                  ) : (
                    <Input
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      disabled={!isAdmin}
                    />
                    <label className="text-sm ml-4">Admin onl send</label>
                    <input
                      type="checkbox"
                      checked={settingsAdminOnly}
                      onChange={(e) => setSettingsAdminOnly(e.target.checked)}
                      disabled={!isAdmin}
                    />
                  </div>
                  {isAdmin && (
        ...
        <DialogContent className="sm:max-w-md p-6 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) => setCreateAdminOnly(e.target.checked)}
            />
          </div>
